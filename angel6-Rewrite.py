...
class YTDLSource(discord.PCMVolumeTransformer):
    ...
    async def create_source(cls, ctx: commands.Context, search: str, *, loop: asyncio.BaseEventLoop = None):
        ...
        return cls(ctx, discord.FFmpegPCMAudio(info['url'], executable="ffmpeg-5.0.1/ffmpeg.exe", **cls.FFMPEG_OPTIONS), data=info)
    ...
    def parse_duration(duration: int):
        ...
        duration_formated = []
        if days > 0:
            duration_formated.append('{}'.format(days))
        if hours > 0:
            duration_formated.append('{}'.format(hours))
        if minutes > 0:
            duration_formated.append('{}'.format(minutes))
        if seconds > 0:
            duration_formated.append('{}'.format(seconds))
    ...
    async def audio_player_task(self):
        while True:
            ...
            # If the song is looped
                ...
                self.now = discord.FFmpegPCMAudio(self.current.source.stream_url, executable="ffmpeg-5.0.1/ffmpeg.exe", **YTDLSource.FFMPEG_OPTIONS)
        ...
        async with ctx.typing():
            try:
                source = await YTDLSource.create_source(ctx, search,
                                                        loop=bot.loop)  # I don't think self is needed here, removed
...
async def ping(ctx):
    """shows the ping"""
    embed = discord.Embed(title="Bot latency:", color=discord.Color.blurple(),
...
async def on_ready():
    ...
    if os.getenv("JOIN_LEAVE_CHANNEL_ID") == "":
        ...
        with open(".env", "r") as envfile:
            content1 = envfile.read()
            changed = content1.replace("JOIN_LEAVE_CHANNEL_ID=",
                                       ''.join(["JOIN_LEAVE_CHANNEL_ID=", str(joinleaveinput)]))
            with open('.env', 'w') as envfile:
                envfile.write(changed)
        restartbot = True
    ...
    embed = discord.Embed(title='Bot settings', description='Current bot settings and status',
                          color=discord.Color.blurple())
...
async def userinfo(ctx, *, user: discord.Member = None):  # b'\xfc'
    ...
    perm_string = ', '.join([str(p[0]).replace("_", " ").title() for p in user.guild_permissions if
                             p[1]])  # I don't like the guild permissions part, way too much info, useless
    ...
    if not mutedRole:
        ...
        for channel in guild.channels:
            await channel.set_permissions(mutedRole, speak=False, send_messages=False, read_message_history=True,
                                          read_messages=True, create_private_threads=False, create_public_threads=False)
...
async def stats(ctx):
    """shows bot stats"""
    bedem = discord.Embed(title='System Resource Usage and statistics', description='See bot host statistics.',
                          color=discord.Color.blurple())
...
async def ban(ctx, member: discord.Member = None, *, reason=None):
    ...
    try:
        ...
        await ctx.reply(
            "Can't ban the member, make sure the bot is higher on the role list and that the bot has the necessary permissions. ")
...
async def IQ(ctx):
    """Average server IQ"""
    embed = discord.Embed(title=f"Average {ctx.guild.name} IQ", description=f"{random.randint(-10, 130)}",
                          color=discord.Color.blurple())
    await ctx.reply(embed=embed)
...
async def credit(ctx):
    ...
    embed = discord.Embed(title=f"Made by: {owner}, Maintained by: {maintainer}",
...
async def femboy(ctx):
    ...
    embed = discord.Embed(title="Chakal's Wisdom On Femboys",
    ...
    embed2 = discord.Embed(title="Miro's Wisdom On Femboys",
